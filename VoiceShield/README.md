...
This phase delivers a clean, modular project skeleton only:

- Running FastAPI backend with `/health` and route stubs
- Running React + TypeScript frontend with all pages
- Audio selection UI (upload + browser recording) with validation
- Reusable visualization components (waveform, audio player, spectrogram placeholder)
- PostgreSQL/SQLAlchemy database foundation

**No AI predictions are generated in this phase.** Values display
"Not analyzed" until the ML phases arrive.
...
| Database   | PostgreSQL                                                       |
| AI/ML      | PyTorch, Hugging Face Transformers, Wav2Vec2, SpeechBrain, ECAPA-TDNN (future phases) |
| Audio      | FFmpeg, Librosa, Torchaudio, NumPy, SoundFile (future phases)    |
...
├── frontend/
│   └── src/
│       ├── components/   # Waveform, player, spectrogram, badges
│       ├── pages/        # Landing, Dashboard, Analyze, History, Profile
│       ├── layouts/      # App shell
│       ├── services/     # api, audio, analysis, history, profile
│       ├── hooks/        # useRecorder, useBackendHealth
│       ├── types/        # audio, analysis, api
│       ├── utils/        # validation, formatting
│       └── assets/       # static assets
├── backend/
│   └── app/
│       ├── api/routes/   # health, audio, analysis, history, profile
│       ├── core/         # config, exceptions
│       ├── database/     # SQLAlchemy engine/session/base
│       ├── models/       # ORM models (future phase)
│       ├── schemas/      # Pydantic schemas
│       ├── services/     # Business logic (future phase)
│       ├── ml/           # ML abstractions (future phase)
│       ├── audio/        # Audio pipeline (future phase)
...
- **Phase 1 (current):** Project foundation — pages, routing, services,
  audio selection UI, health endpoint, database foundation.
- **Phase 2:** Audio upload + preprocessing, and detection pipeline.
